...
export async function generateMetadata(props: PageProps<"/[locale]/calculator/finance/pay">): Promise<Metadata> {
  ...
  return {
    title: t("payCalcCalculator"),
    description: t("payCalcCalculatorDesc"),
    keywords: ["pay calculator", "salary after tax calculator", "net pay calculator"],
    alternates: { canonical: `https://easycalcfor.me/${locale}/calculator/finance/pay` },
  };
}

export default async function Page(props: PageProps<"/[locale]/calculator/finance/pay">) {
  const { locale } = await props.params;
  const t = await getTranslations({ locale, namespace: "calculators" });
  ...
  return (
    <>
      <CalculatorJsonLd name={t("payCalcCalculator")} description={t("payCalcCalculatorDesc")} url={`${baseUrl}/${locale}/calculator/finance/pay`} locale={locale} faqItems={[{ question: t("paycheckFaq1Q"), answer: t("paycheckFaq1A") }, { question: t("paycheckFaq2Q"), answer: t("paycheckFaq2A") }]} />
      <BreadcrumbJsonLd items={[{ name: "Home", url: `${baseUrl}/${locale}` }, { name: tCat("financePageTitle"), url: `${baseUrl}/${locale}/calculator/finance` }, { name: t("payCalcCalculator"), url: `${baseUrl}/${locale}/calculator/finance/pay` }]} />
