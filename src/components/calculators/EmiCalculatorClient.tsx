...
export default function EmiCalculatorClient() {
  ...
  return (
    <div className="bg-card border rounded-2xl p-6 shadow-sm max-w-sm space-y-4">
      <div>
        <Label>{t("emiLoanAmount")}</Label>
        <Input
          type="number"
          placeholder={`${t("egPrefix")} 1000000`}
          value={loanAmount}
      ...
      <div>
        <Label>{t("emiTenure")}</Label>
        <div className="flex gap-2 mt-1">
          <Input
            type="number"
            placeholder={tenureUnit === "years" ? `${t("egPrefix")} 20` : `${t("egPrefix")} 240`}
            value={tenure}
      ...
      {result && (
        <div className="mt-2 space-y-4">
          <div className="p-4 rounded-xl bg-primary/10">
            <p className="text-sm text-muted-foreground mb-1">{t("emiMonthlyEmi")}</p>
